...
class Sensor:
    def __init__(self, stream, batch_size = config.BATCH_SIZE, reuse_size = config.REUSE_SIZE):
        ...
        self._data_fetching_thread = Thread(target=_data_fetching_loop, args=(stream, batch_size, self.data_queue))
        self._data_fetching_thread.start()

        log_message(0, repr(self._data_fetching_thread))
...
# Converts and accumulates the raw output from the sensor to a Pandas data frame
def _data_fetching_loop(stream, data_queue, batch_size):
    try:
        while True:
            ...
            for line in raw_bytes:
                try:
                    ...
                    if line.startswith("mugicdata"): # Keep only lines with mugicdata prefix
                        samples.append(line.split(' ')[1:])  # but lose that prefix
            ...
            samples = pd.DataFrame(samples, columns = config.COLUMNS, dtype = "double").set_index("time_sec")
